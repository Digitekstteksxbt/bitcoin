    ...
    CRITICAL_BLOCK(cs_wallet)
    {
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                boost::trim(sendFromAddressRestriction);
                boost::split(sendFromAddresses, sendFromAddressRestriction, boost::is_any_of(";"));
                if (!sendFromAddressRestriction.empty() && !sendFromAddresses.count(((CWalletTx *)pcoin)->GetAddressOfTxOut(i)))
                    continue;
